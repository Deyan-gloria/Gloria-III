...
// pagos.js - CRUD básico de pagos con filtros
const P_KEY = 'gloria_pagos_v1';
...
document.getElementById('nuevoPago').addEventListener('click', ()=>{
  ...
  const concepto = prompt('Concepto (admin/arriendo/parqueo/otro)');
  const monto = prompt('Monto');
  const fecha = prompt('Fecha (YYYY-MM-DD)', new Date().toISOString().slice(0,10));
  const estado = prompt('Estado (al-dia/en-mora)','al-dia');
  const list = loadP();
